class PersonalTest {
    constructor(target) {
        ...
        this.questions = {
            PF: [{ question: '여행 가자! 어떻게 여행할지 조사해서 정리해볼까?', answer: { a:'YES', b: 'NO'} },{ question: '가서 무슨 일이 생길지 몰라! 이것저것 다 챙기자!', answer: { a:'YES', b: 'NO'} },{ question: '현지에 도착해보니 내 예상과 너무 다르다! 여행은 이런 맛이지!', answer: { a:'YES', b: 'NO'} }],
    ...
    render() {
        ...
        if (this.page === 0) {
            ...
            // render() 메서드 내에서 이미지 업데이트 부분
            const personImageElement = this.container.querySelector('#personImage'); // 이미지 요소 선택

            // 이미지를 변경하는 로직 추가
            if (calcResult === 'PSRH') {
                personImageElement.src = './src/images/84.png'; // '기안84'인 경우 84.png 이미지로 변경
            } else if (calcResult === 'PSRA') {
                personImageElement.src = './src/images/덱스.png'; // '덱스'인 경우 덱스.png 이미지로 변경
            } else {
                personImageElement.src = ''; // 그 외의 경우 이미지를 비움
            }
